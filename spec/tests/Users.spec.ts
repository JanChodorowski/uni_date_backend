...
describe('Users Routes', () => {
    ...
    it(`should return a JSON object with all the users and a status code of "${OK}" if the
      ...
      agent.get(getUsersPath)
        .end((err: Error, res: IResponse) => {
          ...
          const retUsers: User[] = respUsers.map((user: IUser) => new User(user));
  ...
  describe(`"POST:${addUsersPath}"`, () => {
    const callApi = (reqBody: IReqBody) => agent.post(addUsersPath).type('form').send(reqBody);
  ...
  describe(`"PUT:${updateUserPath}"`, () => {
    const callApi = (reqBody: IReqBody) => agent.put(updateUserPath).type('form').send(reqBody);
